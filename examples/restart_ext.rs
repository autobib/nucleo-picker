...
fn main() -> io::Result<()> {
    ...
    spawn(move || {
        const NUM_ITEMS: NonZero<usize> = NonZero::new(1000).unwrap();
        ...
        let mut state = Some((injector, NUM_ITEMS));
        ...
        loop {
            match observer.try_recv() {
                Ok(new_injector) => {
                    ...
                    // it instead
                    state = Some((new_injector, NUM_ITEMS));
                }
                Err(TryRecvError::Empty) => {
                    if let Some((injector, remaining_items)) = state.as_mut() {
                        // we still have remaining data to be sent; continue to send it to the
                        // picker
                        injector.push(slow_random());

                        // if there are no remaining items, drop the injector to mark this
                        // input stream as complete
                        match NonZero::new(remaining_items.get() - 1) {
                            Some(nz) => *remaining_items = nz,
                            None => state = None,
                        }
                    } else {
                        ...
                        // block and wait for the next restart event
                        match observer.recv() {
                            Ok(new_injector) => {
                                // we received the new injector: reset the state and begin
                                // generating again.
                                state = Some((new_injector, NUM_ITEMS));
                            }
                            Err(RecvError) => {
